...
public class Ch4Seeder extends AbstractChapterSeeder {
    ...
    public void seed() {
        ...
        q("ch4-q6","The Covenant of Contracts","Chapter IV · Quest 6","Interfaces",4,6,200,"Contracts.java",
          story(
            n("The Hall of Covenants. Every great guild operates by contract — a set of promises that any member must uphold. It doesn't matter how each member fulfils them, only that they do. This is an interface."),
            d("🧙","mentor","Master Velan","s-mentor","An <em>interface</em> is a pure contract — a list of method signatures with no implementation. Any class can <em>implement</em> multiple interfaces. This is Java's answer to multiple inheritance. Interfaces define <em>what</em> a class can do, not <em>how</em>."),
            e("Worked Example — Interface",
              "<span class='cm'>// The contract</span>\n<span class='kw'>interface</span> <span class='type'>Drawable</span> {\n    <span class='kw'>void</span> draw();  <span class='cm'>// no body — just the promise</span>\n}\n\n<span class='cm'>// A class fulfilling the contract</span>\n<span class='kw'>class</span> <span class='type'>Circle</span> <span class='kw'>implements</span> <span class='type'>Drawable</span> {\n    @Override\n    <span class='kw'>public void</span> draw() {\n        <span class='kw'>System</span>.out.println(<span class='str'>\"Drawing a circle\"</span>);\n    }\n}"),
            d("🧝","npc","Enchantress Lyra","s-npc","The crucial rule: use the interface type on the left side of the variable. <code>Drawable d = new Circle();</code> — not <code>Circle d = new Circle();</code>. This is how you write flexible code. If you later swap Circle for Triangle, nothing else breaks."),
            e("Worked Example — Interface type on left",
              "<span class='type'>Drawable</span> d = <span class='kw'>new</span> <span class='type'>Circle</span>();\nd.draw();  <span class='cm'>// Drawing a circle</span>\n\n<span class='cm'>// A class implementing two interfaces:</span>\n<span class='kw'>interface</span> <span class='type'>Describable</span> {\n    <span class='kw'>String</span> describe();\n}\n\n<span class='kw'>class</span> <span class='type'>Square</span> <span class='kw'>implements</span> <span class='type'>Drawable</span>, <span class='type'>Describable</span> {\n    <span class='kw'>public void</span> draw() { System.out.println(<span class='str'>\"Drawing a square\"</span>); }\n    <span class='kw'>public</span> <span class='type'>String</span> describe() { <span class='kw'>return</span> <span class='str'>\"I am a square\"</span>; }\n}"),
            d("🧙","mentor","Master Velan","s-mentor","Abstract class vs interface: abstract classes can have fields and implemented methods, but a class can only extend one. Interfaces have no state (no instance fields), but a class can implement many. When in doubt: prefer interfaces for shared behaviour contracts.")
          ),
          "Define interface <strong>Describable</strong> with method <code>String describe()</code>.<br>Create <strong>Wizard</strong> and <strong>Dragon</strong> implementing it.<br>• Wizard(\"Aldric\").describe() → <strong>\"Wizard: Aldric\"</strong><br>• Dragon(\"Skaroth\").describe() → <strong>\"Dragon: Skaroth\"</strong><br><br>Store both as <code>Describable</code> type and call describe.",
          "Declare interface Describable with String describe(); — no body. Both classes implement it. Store as: Describable d = new Wizard(\"Aldric\");",
          "// Define Describable interface\n\n// Wizard implements Describable\n\n// Dragon implements Describable\n\npublic class Contracts {\n    public static void main(String[] args) {\n        Describable w = new Wizard(\"Aldric\");\n        Describable d = new Dragon(\"Skaroth\");\n        System.out.println(w.describe());\n        System.out.println(d.describe());\n    }\n}\n",
          "\"Both honour the contract,\" the Hall intones. \"One interface — two implementations. This is how real Java is written.\"",
          tests(test("Wizard","null","Wizard: Aldric"),test("Dragon","null","Dragon: Skaroth")));

        q("ch4-q7","The Arcane Registry","Chapter IV · Quest 7","Enums",4,7,190,"ArcaneRegistry.java",
          story(
            n("The Arcane Registry. Ranks in the Academy are fixed and known — they don't change at runtime. Using plain Strings for them is risky: a typo like \"APPRETNICE\" compiles fine but breaks the logic. Enums solve this."),
            d("🧙","mentor","Master Velan","s-mentor","An <em>enum</em> (enumeration) is a special type that holds a fixed set of named constants. Once defined, only those values are valid — the compiler enforces it. Enums are a smarter alternative to magic strings or integer codes."),
            e("Worked Example — Enum",
              "<span class='kw'>enum</span> <span class='type'>Season</span> { SPRING, SUMMER, AUTUMN, WINTER }\n\n<span class='type'>Season</span> s = <span class='type'>Season</span>.SUMMER;\n<span class='kw'>System</span>.out.println(s);           <span class='cm'>// SUMMER</span>\n<span class='kw'>System</span>.out.println(s.name());    <span class='cm'>// SUMMER</span>\n<span class='kw'>System</span>.out.println(s.ordinal()); <span class='cm'>// 1 (zero-based position)</span>"),
            d("🧝","npc","Enchantress Lyra","s-npc","Enums work perfectly in switch statements. And you can call values() to get an array of all enum constants, which makes looping over them straightforward."),
            e("Worked Example — Enum in switch & loop",
              "<span class='kw'>enum</span> <span class='type'>Rank</span> { NOVICE, APPRENTICE, ADEPT, MAGE, ARCHMAGE }\n\n<span class='type'>Rank</span> r = <span class='type'>Rank</span>.ADEPT;\n<span class='kw'>switch</span> (r) {\n    <span class='kw'>case</span> NOVICE:    System.out.println(<span class='str'>\"Just starting\"</span>); <span class='kw'>break</span>;\n    <span class='kw'>case</span> ADEPT:     System.out.println(<span class='str'>\"Getting skilled\"</span>); <span class='kw'>break</span>;\n    <span class='kw'>default</span>:        System.out.println(<span class='str'>\"High rank\"</span>);\n}\n\n<span class='cm'>// Loop all values</span>\n<span class='kw'>for</span> (<span class='type'>Rank</span> rank : <span class='type'>Rank</span>.values()) {\n    System.out.println(rank);\n}")
          ),
          "Define enum <strong>Rank</strong> with values: <strong>NOVICE, APPRENTICE, ADEPT, MAGE, ARCHMAGE</strong>.<br><br>In main:<br>1. Print <code>Rank.MAGE</code> → <strong>MAGE</strong><br>2. Print its ordinal → <strong>3</strong><br>3. Use a loop to print all 5 values on separate lines",
          "enum Rank { NOVICE, APPRENTICE, ADEPT, MAGE, ARCHMAGE } — define outside the main class. ordinal() gives the zero-based position. Use for (Rank r : Rank.values()) to loop.",
