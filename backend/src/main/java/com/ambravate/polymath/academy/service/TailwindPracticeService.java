...
/**
 * Validates student Tailwind HTML submissions by parsing the markup with Jsoup
 * and checking that required CSS classes are present on the correct elements.
 *
...
public class TailwindPracticeService {
    ...
    public SubmitResponse submit(String userId, String subChunkId, String html) {
        ...
        for (Map<String, Object> spec : specs) {
            ...
            if (!hasElement) {
                ...
                actual = "classes: " + matched.first().className();
            }
        ...
        if (allPassed) {
            ...
            log.info("[Tailwind] All tests passed | user={} subChunk={} xp={}", userId, subChunkId, xpEarned);
        } else {
            log.info("[Tailwind] Tests failed | user={} subChunk={} failures={}",
                    userId, subChunkId,
     ...
     *
     * Idempotency: when XP is first awarded we set lastScore = 1.0 on the progress
     * record as a "guided-practice-xp-awarded" marker. Subsequent calls see this
     * flag and return 0.
     */
