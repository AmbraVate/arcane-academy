...
- **Displaying raw technical error messages to users**: API error objects often contain stack traces, error codes, or SQL messages. Always translate errors into user-friendly language that explains what happened and what to do next.
- **Not providing a retry action**: An error message with no "Try again" button leaves users with no path forward. Nearly every transient error state should offer a retry mechanism.
- **Clearing user data when an error occurs**: If a form submission fails, clearing the form forces the user to re-type everything. Preserve all form data on error so retrying is a single button click.
- **Omitting `role="alert"` on error messages**: Error messages rendered dynamically must use `role="alert"` or `aria-live="assertive"` to be announced immediately to screen reader users. Visible-only error states are inaccessible.
- **Treating all errors identically**: A network failure (retryable), a 404 (content missing), and a 403 (permissions) each require different copy and different recovery actions. Generic "Something went wrong" messages fail users who need specific guidance.
