...
- **Putting content between `<textarea>` tags**: In React, `<textarea>value here</textarea>` does not set the controlled value. Use `value={text}` and `onChange`, exactly like a text input.
- **Using `selected` on `<option>` elements**: In plain HTML, the selected option is marked with the `selected` attribute. In React, set `value={choice}` on the `<select>` element itself — `selected` on individual options is ignored.
- **Comparing a number state value to a string option value**: If state holds the number `2` and the option has `value="2"`, they will not match and the dropdown will appear blank. Keep value types consistent (always strings for select values).
- **Reading `e.target.value` for multi-select**: For a `multiple` select, `e.target.value` only returns the last selected option. Use `Array.from(e.target.selectedOptions, o => o.value)` to get all selected values.
