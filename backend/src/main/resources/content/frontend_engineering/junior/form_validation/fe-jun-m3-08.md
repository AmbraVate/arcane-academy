...
- **Using vague error messages like "Invalid input"**: Users need to know what is wrong and how to fix it. "Password must be at least 8 characters" is useful; "Invalid" is not.
- **Placing error messages far from the field they describe**: Error text should appear immediately adjacent to the field it refers to — not at the top of the form, not in a tooltip. Proximity reduces the cognitive work of matching errors to fields.
- **Omitting `aria-invalid` and `aria-describedby`**: Without these ARIA attributes, screen reader users hear no indication that a field is in error or what the error says. Accessibility compliance requires these on every erroneous input.
- **Showing all errors at once on first render**: Displaying errors before the user has interacted with the form (because `errors` is initialized with a full validation run) overwhelms the user before they have started. Track a `touched` state and show errors only after interaction.
