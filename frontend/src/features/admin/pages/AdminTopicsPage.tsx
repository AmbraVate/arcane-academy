...
function TopicForm({
  ...
  onSave: (t: Partial<AdminTopic>) => void
  onCancel: () => void
}) {
  const [form, setForm] = useState<Partial<AdminTopic>>(initial)
  const set = (k: keyof AdminTopic, v: unknown) => setForm(prev => ({ ...prev, [k]: v }))
...
export default function AdminDomainsPage() {
  ...
  async function handleSave(form: Partial<AdminTopic>) {
    if (!form.name?.trim()) { setError('Name is required'); return }
    setSaving(true); setError(null)
    try {
      if (editTopic) {
        await adminTopicApi.update(editTopic.id, form)
      } else {
        if (!form.id?.trim()) { setError('ID is required for new topics'); setSaving(false); return }
        await adminTopicApi.create(form as AdminTopic)
      }
  ...
  async function handleDelete(topic: AdminTopic) {
    ...
    setError(null)
    try {
      await adminTopicApi.delete(topic.id)
      load()
    } catch (e: unknown) {
