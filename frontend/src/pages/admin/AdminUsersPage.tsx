...
import { adminUserApi, type AdminUser, type UserStats } from '../../api/adminServices'
  ...
  return (
    <div style={{ ...s.card, width: 280, flexShrink: 0, padding: 20, alignSelf: 'flex-start', position: 'sticky', top: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <div style={{ fontFamily: 'Cinzel, serif', fontSize: 14, color: '#c4b5fd', fontWeight: 700 }}>
            {user.username}
          </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        <span style={{
          fontSize: 10, fontFamily: 'Cinzel, serif', padding: '3px 8px', borderRadius: 5,
          background: ROLE_COLOR[user.role] + '22',
          border: `1px solid ${ROLE_COLOR[user.role]}44`,
          color: ROLE_COLOR[user.role],
        }}>{user.role}</span>
        {user.blocked && (
          <span style={{ fontSize: 10, fontFamily: 'Cinzel, serif', padding: '3px 8px', borderRadius: 5, background: 'rgba(248,113,113,.12)', border: '1px solid rgba(248,113,113,.3)', color: '#f87171' }}>
            BLOCKED
          </span>
        )}
        <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 5, background: 'rgba(139,92,246,.1)', border: '1px solid rgba(139,92,246,.25)', color: '#c4b5fd' }}>
          {user.authProvider}
        </span>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, ...s.label }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <span style={{ fontSize: 11, color: '#8b7fa0' }}>
      ...
      {error && (
        <div style={{ fontSize: 11, color: '#f87171', background: 'rgba(248,113,113,.1)', border: '1px solid rgba(248,113,113,.25)', borderRadius: 6, padding: '6px 10px', marginBottom: 12 }}>
          {error}
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingTop: 12, borderTop: '1px solid #2e2850' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {(['USER', 'ADMIN'] as const).map(role => (
              <button
                key={role}
                disabled={isSelf || roleSaving || user.role === role}
                onClick={() => handleRoleChange(role)}
                style={{
                  flex: 1, fontSize: 11, fontFamily: 'Cinzel, serif', padding: '6px 8px', borderRadius: 6, cursor: isSelf ? 'not-allowed' : 'pointer',
                  background: user.role === role ? ROLE_COLOR[role] + '22' : 'transparent',
                  border: `1px solid ${ROLE_COLOR[role] + (user.role === role ? '66' : '33')}`,
                  color: user.role === role ? ROLE_COLOR[role] : '#8b7fa0',
                  transition: 'all .15s',
        ...
        {/* Block/Unblock */}
        <button
          disabled={isSelf || blocking}
          onClick={handleBlock}
          style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
            fontSize: 11, padding: '7px 12px', borderRadius: 6, cursor: isSelf ? 'not-allowed' : 'pointer',
            background: user.blocked ? 'rgba(74,222,128,.08)' : 'rgba(248,113,113,.08)',
            border: `1px solid ${user.blocked ? 'rgba(74,222,128,.3)' : 'rgba(248,113,113,.3)'}`,
            color: user.blocked ? '#4ade80' : '#f87171',
            transition: 'all .15s', opacity: isSelf ? 0.5 : 1,
          }}
        >
